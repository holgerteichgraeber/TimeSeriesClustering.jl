...
sys.path.append('clust_algorithms')
import kshape
...
def run_kshape(el_resh, n_clusters, n_init=1,max_iter=100,n_jobs=1):  # el_resh is (days,24)
...
def plot_clusters(cluster_centers,weights,n_clusters,descript, el_resh, closest_days=np.zeros(1), showfigs = True ):
    ...
    if showfigs:
        f.show()
    f.savefig("../outfiles/save_clusters/clust"+str(n_clusters)+"_"+str(descript)+".png")
    f.savefig("../outfiles/save_clusters/clust"+str(n_clusters)+"_"+str(descript)+".eps")
...
def write_clusters_to_txt_battery_opt(cluster_centers, weights, n_clusters,filename_elpr,filename_weight):
    here = os.path.dirname(os.path.realpath(__file__))
    filepath_elpr = os.path.join( "../outfiles/representative_days",filename_elpr)
    filepath_weight = os.path.join( "../outfiles/representative_days",filename_weight)
...
if __name__ == '__main__':
    ...
    ### SETTINGS ###
    n_rand_km = 10#  random starting points for kmeans
    n_k = np.arange(1,10)
    showfigs = False  
    ...
    if region == "CA":
        el_CA_2015 = read_CA_el_data_CSV('../data/el_prices/CA_2015_elPriceStanford.csv')
        reg_str = "" # prefix for datanaming later
    elif region == "GER":
        el_CA_2015 = read_CA_el_data_CSV('../data/el_prices/GER_2015_elPrice.csv')
    ...
    for k in n_k:
        tic = time()
        res = run_kshape(el_CA_2015_norm_meansdv, k,n_init=n_rand_km, max_iter=max_iter, n_jobs=n_jobs)
        toc = time()
        print "k=" + str(k) + " took " + str(toc - tic) + " s  \n" 
