...
function run_opt(ts_data::ClustData,
  ...
  if opt_config["lost_cost"]!=Dict{String,Number}("el_load"=>Inf, "CO2_emission"=>Inf)
  ...
  if opt_config["co2_limit"]!=Inf
    setup_opt_cep_co2_limit!(cep, ts_data, opt_data; co2_limit=opt_config["co2_limit"], lost_cost=opt_config["lost_cost"])
  end
  setup_opt_cep_demand!(cep, ts_data, opt_data; lost_cost=opt_config["lost_cost"])
  ...
  setup_opt_cep_objective!(cep, ts_data, opt_data; lost_cost=opt_config["lost_cost"])
  return solve_opt_cep(cep, ts_data, opt_data, opt_config)
end

"""
function run_opt(ts_data::ClustData,opt_data::OptDataCEP,fixed_design_variables::Dict{String,OptVariable};solver::Any=CbcSolver(),lost_cost::Dict{String,Number}=Dict{String,Number}("el_load"=>Inf, "CO2_emission"=>Inf))
  ...
  lost_cost: A dictionary containing the values for
                  lost load electricity: Number indicating the lost load price/MWh (should be greater than 1e6)
                  lost emission CO2: Number indicating the emission price/kg-CO2 (should be greater than 1e6)
                  give Inf for both for no slack
"""
function run_opt(ts_data::ClustData,
                    opt_data::OptDataCEP,
                    opt_config::Dict{String,Any},
                    ...
                    solver::Any=CbcSolver(),
                    lost_cost::Dict{String,Number}=Dict{String,Number}("el_load"=>Inf, "CO2_emission"=>Inf),
                    lost_emission_cost::Number=Inf,
                    k_ids::Array{Int64,1}=Array{Int64,1}())
  # Add the fixed_design_variables and new setting for slack costs to the existing config
  set_opt_config_cep!(opt_config;fixed_design_variables=fixed_design_variables, lost_cost=lost_cost)
  return run_opt(ts_data,opt_data,opt_config;solver=solver,k_ids=k_ids)
end

"""
function run_opt(ts_data::ClustData,opt_data::OptDataCEP,fixed_design_variables::Dict{String,OptVariable};solver::Any=CbcSolver(),descriptor::String="",   ,co2_limit::Number=Inf,lost_cost::::Dict{String,Number}=Dict{String,Number}("el_load"=>Inf, "CO2_emission"=>Inf),existing_infrastructure::Bool=false, intrastorage::Bool=false)
...
function run_opt(ts_data::ClustData,
  ...
  opt_config=set_opt_config_cep(opt_data; descriptor=descriptor, co2_limit=co2_limit, lost_cost=lost_cost, existing_infrastructure=existing_infrastructure, limit_infrastructure=limit_infrastructure, storage_e=storage, storage_p=storage, interstorage=interstorage, transmission=transmission, print_flag=print_flag)
